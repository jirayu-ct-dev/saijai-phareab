...
import { backdatedOrderSchema, backdatedSaleSchema } from "../../shared/utils/backdatedOrder";
...
describe("backdated package sale", () => {
  const saleSchema = backdatedSaleSchema(new Date("2026-09-05T05:00:00Z"));
  const sold = { soldAt: "2026-09-01T09:00" };

  it("interprets local input in Bangkok and keeps sale and payment independent", () => {
    const result = saleSchema.parse({ ...sold, payment: { paidAt: "2026-09-01T10:00", method: "TRANSFER" } });
    expect(result.soldAt.toISOString()).toBe("2026-09-01T02:00:00.000Z");
    expect(result.payment?.paidAt.toISOString()).toBe("2026-09-01T03:00:00.000Z");
  });

  it.each([
    { soldAt: "2026-09-06T12:00" },
    { soldAt: "2026-02-30T12:00" },
    { payment: { paidAt: "2026-08-31T12:00", method: "CASH" } },
    { payment: { paidAt: "2026-09-06T12:00", method: "CASH" } },
    { payment: { paidAt: "2026-09-02T12:00", method: "OTHER" } },
  ])("rejects invalid historical facts: %j", (change) => {
    expect(saleSchema.safeParse({ ...sold, ...change }).success).toBe(false);
  });

  it("allows outstanding payment", () => {
    expect(saleSchema.parse(sold).payment).toBeUndefined();
  });
});
