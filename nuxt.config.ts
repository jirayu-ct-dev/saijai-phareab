...
const printGatewayOrigin = (() => {
  try {
    if (process.env.NUXT_PUBLIC_PRINT_GATEWAY_ENABLED !== 'true') return null
    const value = process.env.NUXT_PUBLIC_PRINT_GATEWAY_URL
...
const contentSecurityPolicy = [
  ...
  "font-src 'self' data:",
  `connect-src 'self' https://api.line.me${printGatewayOrigin ? ` ${printGatewayOrigin}` : ''}${isDev ? ' ws://localhost:* http://localhost:*' : ''}`,
...
export default defineNuxtConfig({
  ...
  nitro: {
    ...
    serverAssets: [{
      baseName: 'printing',
      dir: '../public',
      pattern: '{logo-saijai-phareab.png,fonts/Prompt-normal-400-{thai,latin}.woff2,fonts/Prompt-normal-700-{thai,latin}.woff2}',
    }],
    routeRules: {
      '/**': {
        headers: {
