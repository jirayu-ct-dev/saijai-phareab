...
> `.env.example` ยังไม่มี `LINE_CHANNEL_ID`, `LINE_CHANNEL_SECRET`, `CLOUDINARY_API_KEY` และ `CLOUDINARY_API_SECRET` แต่โค้ด integration บางส่วนต้องใช้ค่าเหล่านี้ ให้เพิ่มใน `.env` เมื่อต้องใช้งานบริการดังกล่าว และห้าม commit secret ลง repository
