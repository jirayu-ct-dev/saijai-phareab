...
const totalRows = computed<InfoRow[]>(() => {
  ...
  if (hasMemberEntitlement.value && order.value?.memberEntitlement) {
            ...
            <div v-if="hasMemberEntitlement" class="min-h-28 bg-default p-3! dark:bg-elevated/55 sm:col-span-3 sm:rounded-lg sm:border sm:border-default/30 sm:dark:border-default/20">
              <div class="flex min-w-0 items-start justify-between gap-3">
                <div class="min-w-0 space-y-1">
                  ...
                  <p class="truncate text-lg font-semibold text-highlighted">{{ order?.memberEntitlement?.productName || "ใช้สิทธิ์แพ็กเกจ" }}</p>
                  <p class="text-xs text-muted">
                    ใช้ {{ usedCreditLabel }}
                    <template v-if="order?.memberEntitlement?.creditRemaining != null">
                      · คงเหลือ
                      <span :class="Number(order.memberEntitlement.creditRemaining) < 0 ? 'font-medium text-error' : ''">
                        {{ Number(order.memberEntitlement.creditRemaining) < 0 ? `ติดลบ ${Math.abs(Number(order.memberEntitlement.creditRemaining))} เครดิต` : `${order.memberEntitlement.creditRemaining} เครดิต` }}
                      </span>
                    </template>
                    <span v-if="order?.memberEntitlement?.isSettled" class="text-info font-medium"> (หักจากแพ็กใหม่แล้ว)</span>
                  </p>
                </div>
                <div class="flex size-9 shrink-0 items-center justify-center rounded-lg bg-success/10 text-success">
                      ...
                      <div class="flex min-w-0 items-start justify-between gap-2">
                        <div class="min-w-0">
                          <div class="flex min-w-0 flex-wrap items-center gap-1.5">
                            ...
                            <UBadge v-else-if="order.memberEntitlement && item.isChargeable" color="primary" variant="subtle" size="xs">
                              นอกแพ็กเกจ · คิดเงิน
                            </UBadge>
                          ...
                          <div class="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-muted">
                            ...
                            <span>{{ item.isPackageIncluded || !item.isChargeable ? "-" : formatCurrency(item.unitPrice) }}</span>
                          </div>
                        </div>
                        <div class="shrink-0 text-right">
                          ...
                          <p v-else-if="item.isChargeable" class="text-[13px] font-semibold leading-none tabular-nums text-primary">{{ formatCurrency(item.totalPrice) }}</p>
                  ...
                  <tbody class="[&>tr:last-child>td]:border-b-0">
                    ...
                    >
                      ...
                      <td class="border-b border-default px-3 py-2 text-right dark:border-default/25">
                        ...
                        <span v-else-if="item.isChargeable" class="font-semibold text-highlighted">{{ formatCurrency(item.totalPrice) }}</span>
