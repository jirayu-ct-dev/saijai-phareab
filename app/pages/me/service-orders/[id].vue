  ...
  <div class="flex-1 flex flex-col min-h-0">
    <UDashboardPanel grow>
      <UDashboardNavbar :title="order?.orderNo ? `ออเดอร์ ${order.orderNo}` : 'รายละเอียดออเดอร์'">
        ...
        <template #right>
          <UButton color="neutral" variant="ghost" to="/me/service-orders" icon="i-lucide-arrow-left">กลับ</UButton>
        </template>
      </UDashboardNavbar>
      ...
      <ClientOnly>
        ...
        <div v-else-if="!order" class="p-12 text-center">
          ...
          <UButton to="/me/service-orders" color="neutral" class="mt-4">กลับไปรายการออเดอร์</UButton>
        </div>
        ...
        <template #fallback>
          <div class="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p class="text-sm text-muted">กำลังโหลดรายละเอียดออเดอร์...</p>
          </div>
        </template>
      </ClientOnly>
    </UDashboardPanel>
