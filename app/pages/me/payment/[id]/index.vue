...
import type { PackageSaleStatus, PaymentMethod, PaymentStatus, ServiceOrderStatus } from "~~/shared/types/enums";
...
type PaymentDetailResponse = {
  ...
  amount: number;
  note: string | null;
  vat: { rate: number; amount: number; included: boolean; baseAmount: number } | null;
  ...
  serviceOrder: {
    ...
    items: Array<{
      ...
      photos: Array<{ id: string; imageId: string; isDamaged: boolean; sortOrder: number; url: string | null; secureUrl: string | null }>;
    }>;
  } | null;
};
...
const hasError = computed(() => Boolean(error.value) || !payment.value);
...
const openPaymentDocument = () => {
  ...
  void navigateTo(`/me/payment/${payment.value.id}/${target}`);
};
...
const detailItems = computed<DetailItem[]>(() => {
  ...
  if (payment.value.packageSale) {
    return payment.value.packageSale.items.map((item) => ({
      ...
      title: item.name,
      metaLabel: [
        item.credits ? `เครดิต ${item.credits}` : null,
        item.validityDays ? `อายุ ${item.validityDays} วัน` : null,
      ].filter(Boolean).join(" • "),
      unitPriceLabel: formatCurrency(item.unitPrice),
      quantityLabel: `${item.quantity} รายการ`,
      totalLabel: formatCurrency(item.totalPrice),
      badgeLabel: packageTypeLabels[item.type],
      badgeColor: packageTypeColors[item.type] as BadgeColor,
    }));
  }
...
const entitlementRows = computed<InfoRow[]>(() => payment.value?.serviceOrder?.memberEntitlement ? [
  { label: "ชื่อแพ็กเกจ", value: payment.value.serviceOrder.memberEntitlement.productName },
  { label: "เครดิตเริ่มต้น", value: String(payment.value.serviceOrder.memberEntitlement.creditInitial) },
  { label: "เครดิตคงเหลือ", value: String(payment.value.serviceOrder.memberEntitlement.creditRemaining) },
  { label: "หมดอายุ", value: payment.value.serviceOrder.memberEntitlement.endAt ? formatDateTime(payment.value.serviceOrder.memberEntitlement.endAt) : "-" },
] : []);
</script>
  ...
  <UDashboardPanel id="my-payment-detail">
    ...
    <template #body>
      <div class="flex flex-col gap-3 p-2 sm:p-6">
          ...
          <div class="min-w-0 space-y-3">
            <div class="-mx-2 border border-default/30 bg-default p-4 p-5! dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
              ...
              <div class="mt-5 space-y-4">
                <div v-for="section in 2" :key="section" :class="section > 1 ? 'border-t border-default pt-5' : ''">
                  <USkeleton class="h-4 w-32 rounded-lg" />
                  <div class="mt-3 grid gap-x-6 gap-y-2 lg:grid-cols-2">
                    <div v-for="i in 4" :key="`${section}-${i}`" class="flex items-center justify-between gap-3">
        ...
        <div v-else-if="payment" class="grid min-w-0 gap-3 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div class="min-w-0 space-y-3">
            <section class="-mx-2 space-y-3 border border-default/30 bg-default p-4 dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
              ...
              <div class="border-t border-default pt-5">
                <section>
                  <div class="flex items-start justify-between gap-3">
                    ...
                    <UBadge
                      :color="payment.packageSale ? packageSaleStatusMap[payment.packageSale.status].color : serviceOrderStatusMap[payment.serviceOrder!.status].color"
                      variant="subtle"
                      size="sm"
                    >
              ...
              <div v-if="entitlementRows.length" class="border-t border-default pt-5">
                <section>
            ...
            <section class="-mx-2 overflow-hidden border border-default/30 bg-default p-4 dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
              <div class="flex flex-wrap items-center justify-between gap-2 border-b border-default/40 pb-3">
                <p class="text-sm font-semibold text-highlighted">
                  {{ itemSectionTitle }}
                  <span class="ml-2 text-xs text-muted">{{ itemSectionDescription }}</span>
                </p>
              </div>
              <div class="pt-3">
            ...
            <section class="-mx-2 space-y-3 border border-default/30 bg-default p-4 dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
              ...
              <div class="mt-5 space-y-2 text-sm">
                <div
                  ...
                  :class="row.dividerBefore ? 'border-t border-dashed border-default pt-3 mt-3' : ''"
                >
                  <span class="shrink-0 text-muted">{{ row.label }}</span>
            ...
            <section class="-mx-2 space-y-3 border border-default/30 bg-default p-4 dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
              <div class="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p class="text-sm text-muted">รายละเอียดสถานะและเอกสาร</p>
                </div>
              </div>
              ...
              <div class="mt-5 space-y-3 border-t border-default pt-5">
                <div class="rounded-lg border border-default p-4 text-sm">
                  <div class="flex items-start justify-between gap-3">
                    ...
                    <span class="text-right text-highlighted">{{ formatDateTime(payment.updatedAt) }}</span>
                  </div>
                </div>
                <UButton block :label="paymentDocumentLabel" :icon="paymentDocumentIcon" color="primary" variant="solid" @click="openPaymentDocument" />
              </div>
            </section>

            <section v-if="payment.note || payment.slipImage" class="-mx-2 space-y-3 border border-default/30 bg-default p-4 dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
              <div>
                <p class="text-base font-semibold text-highlighted">ข้อมูลเพิ่มเติม</p>
                <p class="text-sm text-muted">หมายเหตุและหลักฐานการชำระเงิน</p>
              </div>

              <p v-if="payment.note" class="rounded-lg border border-default/60 bg-default/45 p-3 text-sm text-muted whitespace-pre-line">{{ payment.note }}</p>

              <NuxtImg
                v-if="payment.slipImage"
                :src="payment.slipImage.secureUrl || payment.slipImage.url || ''"
                class="max-h-96 w-full rounded-lg border border-default object-contain"
                sizes="360px"
                loading="lazy"
              />
            </section>
