...
const { data, status, refresh, error } = await useFetch<ReceiptPayload>(() => `/api/me/receipts/${paymentId.value}`, {
...
const totalUsedCredits = computed(() => data.value?.serviceOrder?.usageHistory.reduce((sum, row) => sum + row.quantity, 0) ?? 0);
      ...
      <template v-if="isMemberOrder && memberEntitlement">
        ...
        <section class="mt-3">
          ...
          <div class="mt-2 space-y-1">
            <div
              v-for="row in data.serviceOrder?.usageHistory"
              :key="row.orderId"
