...
const openEditStatusModal = (): void => {
  editStatusOpen.value = true;
};
const openEditPaymentDialog = (): void => {
  editPaymentOpen.value = true;
};
const goToCustomerDetail = (): void => {
  if (!order.value?.customer?.id) return;
  void navigateTo(`/admin/users/${order.value.customer.id}`);
};
...
<template>
  <div class="contents">
        ...
        <UDashboardNavbar title="รายละเอียดรายการรับผ้า" icon="i-lucide-shopping-basket">
          ...
          <template #right>
            <div class="flex flex-wrap items-center gap-2">
              <UButton label="กลับ" color="neutral" variant="outline" icon="i-lucide-arrow-left" class="shrink-0"
                aria-label="กลับ" :ui="{ label: 'hidden sm:inline' }" @click="goBack" />
              ...
              <UButton v-if="canEditPayment" label="แก้ไขการชำระเงิน" color="primary" variant="subtle"
                ...
                :ui="{ label: 'hidden sm:inline' }" @click="openEditPaymentDialog" />
              <UButton :label="documentLabel" color="neutral" variant="outline" :icon="documentIcon" class="shrink-0"
                :aria-label="documentLabel" :ui="{ label: 'hidden sm:inline' }" @click="openDocument" />
            </div>
          </template>
        </UDashboardNavbar>
      </template>
      ...
      <template #body>
        <div class="flex flex-col gap-3 p-2 sm:p-6">
          <div v-if="showSkeleton" class="space-y-3">
            <div
              class="-mx-2 border border-default/30 bg-default p-5! dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
            ...
            <div class="grid gap-3 xl:grid-cols-[minmax(0,1.3fr)_360px]">
              <div class="space-y-3">
                <div
                  class="-mx-2 space-y-3 border border-default/30 bg-default p-5! dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
                ...
                <div
                  class="-mx-2 overflow-hidden border border-default/30 bg-default p-0! dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
          ...
          <div v-else-if="error || !order"
            class="-mx-2 border border-default/30 bg-default p-6! dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
          ...
          <div v-else class="space-y-3">
            ...
            <section
                ...
                <div class="flex min-w-0 items-center gap-4">
                  ...
                  <div class="min-w-0 space-y-1">
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      <UButton :icon="copiedOrderNo ? 'i-lucide-check' : 'i-lucide-copy'" size="xs"
                        :color="copiedOrderNo ? 'success' : 'neutral'" variant="ghost" @click="() => copyOrderNo()" />
                    </div>
                    <p class="text-sm text-muted">
                      รับงานเมื่อ {{ formatDateTime(order.receivedAt) }}<br>
                      <span v-if="order.status === 'COMPLETED' && (order.completedAt || latestPayment?.paidAt)">ส่งผ้า
                        {{ formatDateTime((order.completedAt || latestPayment?.paidAt)!) }}</span>
              ...
              <div class="border-t border-default pt-5">
                ...
                <div
                  class="mt-3 grid gap-x-6 gap-y-3 text-sm lg:grid-cols-2 lg:[&>*:nth-child(odd)]:pr-4 lg:[&>*:nth-child(even)]:border-l lg:[&>*:nth-child(even)]:border-dashed lg:[&>*:nth-child(even)]:border-default lg:[&>*:nth-child(even)]:pl-4">
                  <div v-for="row in customerRows" :key="`c-${row.label}`"
                  ...
                  <div v-for="row in orderRows" :key="`o-${row.label}`" class="flex items-start justify-between gap-3">
                    ...
                    <span v-else :class="['max-w-[62%] text-right text-highlighted', row.valueClass]">{{ row.value
                      }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="border-t border-default pt-5">
                ...
                <div
                  class="mt-3 grid gap-x-6 gap-y-3 text-sm lg:grid-cols-2 lg:[&>*:nth-child(odd)]:pr-4 lg:[&>*:nth-child(even)]:border-l lg:[&>*:nth-child(even)]:border-dashed lg:[&>*:nth-child(even)]:border-default lg:[&>*:nth-child(even)]:pl-4">
                  <div v-for="row in paymentRows" :key="`p-${row.label}`"
                    class="flex items-start justify-between gap-3">
                    <span class="text-muted">{{ row.label }}</span>
                    <span :class="['max-w-[62%] text-right text-highlighted', row.valueClass]">{{ row.value }}</span>
                  </div>
              ...
              <div class="border-t border-default pt-5">
                ...
                <div class="mt-3 space-y-2 text-sm">
                  <div v-for="row in totalRows" :key="row.label"
            ...
            <section
              class="-mx-2 overflow-hidden border border-default/30 bg-default p-4 dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
              <div
                class="flex flex-col justify-between gap-3 border-b border-default/40 pb-3 sm:flex-row sm:items-center">
                <div>
                ...
                <div v-if="order.image || order.deliveryImage" class="flex items-center gap-3">
                  ...
                  <div class="flex items-center gap-3">
                    <div class="flex flex-col items-center gap-1">
                      <button v-if="order.image" type="button"
                        ...
                        <NuxtImg :src="order.image.secureUrl || order.image.url || ''"
                          class="h-full w-full object-cover" sizes="48px" loading="lazy" />
                      </button>
                      <div v-else
                        class="flex size-12 items-center justify-center rounded-lg border border-dashed border-default bg-muted/10 text-xs text-muted"
                        title="ไม่มีรูปรับผ้า">
                    ...
                    <div class="flex flex-col items-center gap-1">
                      <button v-if="order.deliveryImage" type="button"
                        class="relative block size-12 overflow-hidden rounded-lg cursor-pointer"
                        title="ดูหลักฐานการส่งผ้า"
                        @click="openImagePreview(order.deliveryImage.secureUrl || order.deliveryImage.url, 'หลักฐานการส่งผ้า')">
                        <NuxtImg :src="order.deliveryImage.secureUrl || order.deliveryImage.url || ''"
                          class="h-full w-full object-cover" sizes="48px" loading="lazy" />
                      </button>
                      <div v-else
                        class="flex size-12 items-center justify-center rounded-lg border border-dashed border-default bg-muted/10 text-xs text-muted"
                        title="ไม่มีรูปส่งผ้า">
                        <UIcon name="i-lucide-image-off" class="size-4" />
                      </div>
                ...
                <div v-if="order.weightKg != null" class="mb-3 rounded-lg border border-warning/40 bg-warning/5 p-4">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p v-if="order.washFoldPricePerKgSnapshot != null" class="mt-1 text-xs text-muted">{{
                        formatCurrency(order.washFoldPricePerKgSnapshot) }} / กก.</p>
                    </div>
                    <div class="text-right">
                      <p class="font-semibold text-highlighted">{{ order.weightKg }} กก.</p>
                      <p class="mt-1 text-[13px] font-semibold text-primary">{{ formatCurrency(order.subtotalAmount) }}
                      </p>
                    </div>
                  </div>
                </div>
                <div v-if="order.items.length" class="space-y-1 md:hidden">
                  <div v-for="item in order.items" :key="item.id"
                    ...
                    <div class="flex min-w-0 items-center gap-2 p-2">
                      <div class="shrink-0">
                        <div
                          class="flex size-14 items-center justify-center overflow-hidden rounded-lg border border-default/30 bg-elevated/30 dark:border-default/20 dark:bg-default/80">
                          <button v-if="getItemPhotos(item)[0]" type="button" class="relative size-full overflow-hidden"
                            @click="openImagePreview(getItemPhotos(item)[0]?.secureUrl || getItemPhotos(item)[0]?.url, `${item.label}`)">
                            <NuxtImg :src="getItemPhotos(item)[0]?.secureUrl || getItemPhotos(item)[0]?.url || ''"
                              class="h-full w-full cursor-pointer object-cover" sizes="56px" loading="lazy" />
                            <UBadge v-if="getItemPhotos(item)[0]?.isDamaged" color="error" variant="solid" size="xs"
                              class="absolute left-0.5 top-0.5">!</UBadge>
                          </button>
                      ...
                      <div class="min-w-0 flex-1">
                        ...
                        <p v-if="item.notes" class="mt-1 line-clamp-2 text-xs text-muted whitespace-pre-line">{{
                          item.notes }}</p>
                      </div>
                    </div>
                  </div>
                ...
                <div v-if="order.items.length" class="hidden overflow-x-auto md:block">
                  <table class="w-full min-w-160 text-sm">
                    <thead class="bg-default text-xs text-toned dark:bg-transparent">
                      <tr>
                        <th
                          class="w-20 border-b border-default bg-default px-3 py-2.5 text-left font-semibold uppercase tracking-wide dark:border-default/20 dark:bg-transparent">
                          รูป</th>
                        <th
                          class="border-b border-default bg-default px-3 py-2.5 text-left font-semibold uppercase tracking-wide dark:border-default/20 dark:bg-transparent">
                          รายการ</th>
                        <th
                          class="w-28 border-b border-default bg-default px-3 py-2.5 text-right font-semibold uppercase tracking-wide dark:border-default/20 dark:bg-transparent">
                          ราคา/ชิ้น</th>
                        <th
                          class="w-24 border-b border-default bg-default px-3 py-2.5 text-right font-semibold uppercase tracking-wide dark:border-default/20 dark:bg-transparent">
                          จำนวน</th>
                        <th
                          class="w-28 border-b border-default bg-default px-3 py-2.5 text-right font-semibold uppercase tracking-wide dark:border-default/20 dark:bg-transparent">
                          รวม</th>
                      ...
                      <tr v-for="item in order.items" :key="item.id"
                        ...
                        <td class="border-b border-default px-3 py-2 dark:border-default/25">
                          <div class="flex flex-wrap gap-1">
                            <button v-for="photo in getItemPhotos(item)" :key="photo.id" type="button"
                              ...
                              <NuxtImg :src="photo.secureUrl || photo.url || ''"
                                class="h-full w-full cursor-pointer object-cover" sizes="56px" loading="lazy" />
                              <UBadge v-if="photo.isDamaged" color="error" variant="solid" size="xs"
                                class="absolute left-0.5 top-0.5">!</UBadge>
                            </button>
                            <div v-if="!getItemPhotos(item).length"
    ...
    <ImagePreviewModal v-model:open="previewOpen" :title="previewTitle" :image-url="previewUrl"
      image-alt="รูปหลักฐาน" />

    <EditServiceOrderModal v-model:open="isEditItemsOpen" :order="orderForEdit" @updated="refresh" />

    <EditServiceOrderStatusModal v-model:open="editStatusOpen" :order="orderForEdit" @updated="refresh" />

    <EditPaymentStateModal v-if="latestPayment && canEditPayment" v-model:open="editPaymentOpen"
      :payment-id="latestPayment.id" :payment-no="latestPayment.paymentNo" :amount="Number(latestPayment.amount ?? 0)"
      :status="latestPayment.status" :method="latestPayment.method" :existing-slip="latestPayment.slipImage ?? null"
