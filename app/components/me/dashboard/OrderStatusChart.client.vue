...
type RawRecord = { date: string; received: number; processing: number; completed: number }
type DataRecord = { date: Date; received: number; processing: number; completed: number }

const { width } = useElementSize(cardRef)
const data = ref<DataRecord[]>([])
const isLoading = ref(false)

const COLORS = ['var(--ui-info)', 'var(--ui-warning)', 'var(--ui-success)']

const fetchData = async () => {
  isLoading.value = true
  try {
    const raw = await $fetch<RawRecord[]>('/api/me/dashboard/order-statuses', {
      query: {
    ...
    if (props.period === 'daily') {
      ...
      for (const r of raw) {
        ...
        const existing = weekly.get(key) ?? { date: week, received: 0, processing: 0, completed: 0 }
        existing.received += r.received
        existing.processing += r.processing
      ...
      for (const r of raw) {
        ...
        const existing = monthly.get(key) ?? { date: month, received: 0, processing: 0, completed: 0 }
        existing.received += r.received
        existing.processing += r.processing
        existing.completed += r.completed
...
const y = [
  (d: DataRecord) => d.received,
  (d: DataRecord) => d.processing,
  (d: DataRecord) => d.completed,
]

const total = computed(() =>
  data.value.reduce((sum, d) => sum + d.received + d.processing + d.completed, 0)
)
  ...
  <UCard ref="cardRef" :ui="{ root: 'overflow-visible', body: '!px-0 !pt-0 !pb-3' }">
    <template #header>
      <div class="flex items-start justify-between">
        <div>
          <p class="text-xs text-muted mb-1">ออเดอร์แยกตามสถานะ</p>
