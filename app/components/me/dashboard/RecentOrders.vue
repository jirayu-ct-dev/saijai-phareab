...
const openOrder = async (orderId: string): Promise<void> => {
  await router.push(`/me/service-orders/${orderId}`)
}
...
<template>
  <section class="flex flex-col gap-1">
    <div
      ...
      <UButton to="/me/service-orders" variant="link" color="primary" size="sm" trailing-icon="i-lucide-arrow-right">
        ดูทั้งหมด
      </UButton>
    </div>
          ...
          <div class="flex items-center gap-2 p-2">
            <div class="min-w-0 flex-1">
              <div class="flex min-w-0 items-baseline gap-1.5">
                <span class="truncate text-sm font-medium text-highlighted">{{ order.orderNo ?? `#${order.id.slice(-6)}`
                  }}</span>
              </div>
            ...
            <div class="flex shrink-0 flex-col items-end gap-0.5">
              <span class="text-sm font-semibold text-primary leading-none">{{ formatCurrency(order.totalAmount)
                }}</span>
            </div>
            <div class="flex shrink-0 items-center">
              <UButton icon="i-lucide-eye" size="xs" color="neutral" variant="ghost" aria-label="ดูออเดอร์"
    ...
    <div
      ...
      <UTable :data="orders" :columns="columns" :loading="isPending" :ui="{
