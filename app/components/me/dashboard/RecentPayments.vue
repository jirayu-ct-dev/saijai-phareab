...
const openPayment = async (paymentId: string): Promise<void> => {
  await router.push(`/me/payment/${paymentId}`)
}
...
<template>
  <section class="flex flex-col gap-1">
    <div
      ...
      <UButton to="/me/payment" variant="link" color="primary" size="sm" trailing-icon="i-lucide-arrow-right">
        ดูทั้งหมด
      </UButton>
    </div>
          ...
          <div class="flex items-center gap-2 p-2">
            <div class="min-w-0 flex-1">
              <div class="flex min-w-0 items-baseline gap-1.5">
                <span class="truncate text-sm font-medium text-highlighted">{{ payment.paymentNo ??
                  `#${payment.id.slice(-6)}` }}</span>
              </div>
            ...
            <div class="flex shrink-0 items-center">
              <UButton icon="i-lucide-eye" size="xs" color="neutral" variant="ghost" aria-label="ดูประวัติการชำระเงิน"
                @click.stop="openPayment(payment.id)" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div
      ...
      <UTable :data="data" :columns="columns" :loading="isPending" :ui="{
        ...
        separator: 'h-0',
      }" @select="(_e: Event, row) => openPayment(row.original.id)">
        <template #empty>
          <div
            class="flex flex-col items-center justify-center rounded-lg border border-dashed border-default/30 bg-default/55 px-3 py-5 text-center text-muted dark:border-default/20 dark:bg-elevated/30">
