...
<template>
  <div
    role="button"
    tabindex="0"
    class="group flex h-full cursor-pointer flex-col rounded-2xl border border-default bg-default p-4 text-left transition-colors"
    :class="selected ? 'border-primary bg-elevated/30' : 'hover:bg-elevated/20'"
    @click="emit('select')"
    ...
    <div class="flex min-h-0 flex-1 flex-col">
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0 flex-1">
          <p class="min-h-12 text-base font-medium text-highlighted wrap-break-word">
            {{ title }}
          </p>
        ...
        <UBadge :color="badgeColor" class="min-w-22 flex items-center justify-center self-start" variant="subtle">
          {{ badgeLabel }}
        </UBadge>
      </div>
    </div>
    ...
    <div class="mt-4 flex items-end justify-between gap-3">
      ...
      <div class="text-right">
        ...
        <div class="mt-1 inline-flex items-center gap-1 rounded-md border border-default bg-default px-1 py-1">
          ...
          <p class="min-w-8 text-center font-medium text-highlighted">
            {{ quantity }}
          </p>
