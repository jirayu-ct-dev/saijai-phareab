...
const openPayment = async (paymentId: string): Promise<void> => {
  await router.push(`/admin/payment/${paymentId}`)
}
...
<template>
  <section class="flex flex-col gap-1">
    <div
      ...
      <UButton to="/admin/payment" variant="link" color="primary" size="sm" trailing-icon="i-lucide-arrow-right">
        ดูทั้งหมด
      </UButton>
    </div>
        ...
        <div v-for="payment in data" :key="payment.id"
          ...
          <div class="flex items-center gap-2 p-2">
            ...
            <div class="flex shrink-0 items-center">
              <UIButtonChatLine v-if="payment.customer.lineUserId" :line-user-id="payment.customer.lineUserId" icon-only
                size="xs" />
