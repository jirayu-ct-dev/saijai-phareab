...
const openOrder = (orderId: string): void => {
  void router.push(`/admin/service-orders/${orderId}`)
}
...
<template>
  <section class="flex flex-col gap-1">
    <div
      class="-mx-2 flex items-center justify-between border border-default/30 bg-default px-3 py-2 dark:border-default/20 dark:bg-elevated/55 sm:mx-0 sm:rounded-lg">
          ...
          <div class="flex items-center gap-2 p-2">
            ...
            <div class="min-w-0 flex-1">
              <div class="flex min-w-0 items-baseline gap-1.5">
                ...
                <span class="shrink-0 font-mono text-[10px] text-muted">{{ order.orderNo ?? `#${order.id.slice(-6)}`
                  }}</span>
              </div>
              <div class="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-muted">
                <UBadge :color="orderTypeColors[order.orderType]" variant="subtle" size="xs">{{
                  orderTypeLabels[order.orderType] }}</UBadge>
                <UBadge :color="orderStatusColors[order.status]" variant="subtle" size="xs">{{
                  orderStatusLabels[order.status] }}</UBadge>
                <span>{{ formatDateTime(order.createdAt) }}</span>
              </div>
            </div>
            <div class="flex shrink-0 flex-col items-end gap-0.5">
              <span class="text-sm font-semibold text-primary leading-none">{{ formatOrderAmount(order) }}</span>
              <span v-if="order.hangerCharge" class="text-[10px] text-muted">ไม้แขวน {{
                formatCurrency(order.hangerCharge.total) }}</span>
            </div>
            <div class="flex shrink-0 items-center">
              <UIButtonChatLine v-if="order.customer.lineUserId" :line-user-id="order.customer.lineUserId" icon-only
                size="xs" />
              <UButton icon="i-lucide-eye" size="xs" color="neutral" variant="ghost" aria-label="ดูรายการรับผ้า"
    ...
    <div
      class="hidden overflow-hidden rounded-lg border border-default/30 bg-default dark:border-default/20 dark:bg-elevated/55 md:block">
      <UTable :data="data" :columns="columns" :loading="status === 'pending'" :ui="{
        ...
        separator: 'h-0',
      }" @select="(_e: Event, row) => openOrder(row.original.id)">
        <template #empty>
          <div
            class="flex flex-col items-center justify-center rounded-lg border border-dashed border-default/30 bg-default/55 px-3 py-5 text-center text-muted dark:border-default/20 dark:bg-elevated/30">
